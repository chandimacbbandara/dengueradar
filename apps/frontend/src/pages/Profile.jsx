    ...
    <div className={`dashboard-layout ${!isMoh && user?.role !== 'admin' ? 'citizen-dashboard' : ''}`}>
      ...
      <div className="dashboard-header" style={{ background: 'var(--surface-2)', padding: '32px 0', borderBottom: '1px solid var(--border)' }}>
        <div className="wrap" style={{ display: 'flex', flexDirection: 'column' }}>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text)', marginBottom: '4px' }}>Account Settings</h1>
          <p style={{ fontSize: '14px', color: 'var(--text-2)' }}>Manage your profile and notification preferences</p>
        </div>
      </div>
      ...
      <div className="wrap" style={{ paddingTop: '32px', paddingBottom: '64px', maxWidth: '800px', margin: '0 auto', width: '100%' }}>
        ...
        <div className="card" style={{ padding: '32px', marginBottom: '24px' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)', marginBottom: '24px' }}>Personal Information</h2>
        ...
        <div className="card" style={{ padding: '32px' }}>
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text)', marginBottom: '24px' }}>Account Details</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            <div>
              <p style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '4px' }}>Email Address</p>
              <p style={{ color: 'var(--text)' }}>{user?.email}</p>
            </div>
            <div>
              <p style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '4px' }}>Account Role</p>
              <p style={{ color: 'var(--text)', textTransform: 'capitalize' }}>{user?.role?.replace('_', ' ')}</p>
            </div>
            <div>
              <p style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '4px' }}>Member Since</p>
              <p style={{ color: 'var(--text)' }}>{user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'N/A'}</p>
            </div>
            <div>
              <p style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: '4px' }}>Password</p>
              <button className="btn btn-outline" style={{ padding: '6px 12px', fontSize: '12px', marginTop: '4px' }}>Change Password</button>
            </div>
          </div>
        </div>
